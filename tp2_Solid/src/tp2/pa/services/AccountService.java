...
public class AccountService{
    ...
    public int[] createUserWithAccount(String name, String password, double initialBalance) throws SQLException {
        ...
        try (Connection conn = DBConnexion.getConnection()) {
            ...
            try {
                ...
                int userId = userRepo.createUser(conn, name, password);
                int accountId = accountRepo.createAccount(conn, userId, initialBalance);
                //
                conn.commit();
    ...
    public double getBalance(int accountId) throws SQLException {
        try (Connection conn = DBConnexion.getConnection()) {
            return accountRepo.findBalanceById(conn, accountId);
        }
    }
    ...
    public void changeHolder(User user, String newHolder) throws SQLException {
        try (Connection conn = DBConnexion.getConnection()) {
            ...
            userRepo.updateName(conn, user.getAccount().getIdAccount(), newHolder);
    ...
    public void deleteAccount(User user, int accountId) throws SQLException {
        try (Connection conn = DBConnexion.getConnection()) {
            conn.setAutoCommit(false);
            //Eliminar en BD
            accountRepo.deleteById(conn, accountId);
            //Commit
            conn.commit();
            //Reflejar en memoria
    ...
    public String accountDetails(User user) throws SQLException {
        ...
        try (Connection conn = DBConnexion.getConnection()) {
            double balance = accountRepo.findBalanceById(conn, acctId);
